...
try {
    const file = std.open('/etc/passwd', 'r');
    console.log('SECURITY BREACH: Read /etc/passwd');
    file.close();
} catch (e) {
    console.log('SUCCESS: Cannot access /etc/passwd');
}
"""
        result = sandbox.execute(code)

        # Should be blocked by WASI
...
const fs = require('std');
try {
    const file = std.open('/bin/sh', 'r');
    console.log('SECURITY BREACH: Read /bin/sh');
    file.close();
} catch (e) {
    console.log('SUCCESS: Cannot access /bin/sh');
}
"""
        result = sandbox.execute(code)
    ...
    def test_filesystem_access_allowed_within_app(self, temp_workspace):
        ...
        code = """
const fs = require('std');
const file = std.open('/app/allowed.txt', 'w');
file.puts('This is allowed');
file.close();
console.log('SUCCESS: Created file in /app');
"""
        result = sandbox.execute(code)

        assert result.success is True
        assert "SUCCESS: Created file in /app" in result.stdout
...
class TestJavaScriptFilesystemEscapePrevention:
    ...
    def test_parent_directory_traversal_blocked(self, temp_workspace):
        ...
        # Create another session to try accessing
        other_session = str(uuid.uuid4())
        other_workspace = temp_workspace / other_session
        other_workspace.mkdir(parents=True)
        (other_workspace / "secret.txt").write_text("Secret data")
...
const fs = require('std');
try {{
    const file = std.open('/app/../{other_session}/secret.txt', 'r');
    console.log('SECURITY BREACH: Accessed other session');
    file.close();
}} catch (e) {{
    console.log('SUCCESS: Cannot traverse to other session');
}}
"""
        result = sandbox.execute(code)
...
const fs = require('std');
try {
    const file = std.open('/tmp/escape.txt', 'w');
    console.log('SECURITY BREACH: Wrote to /tmp');
    file.close();
} catch (e) {
    console.log('SUCCESS: Cannot access /tmp');
}
"""
        result = sandbox.execute(code)
