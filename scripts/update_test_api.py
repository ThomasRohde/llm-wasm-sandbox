...
def update_file(file_path: Path) -> None:
    ...
    def replace_create(match):
        ...
        return f'{indent}sandbox = create_sandbox({params})\n{indent}session_id = sandbox.session_id'
    ...
    while i < len(lines):
        ...
        if 'session_id, sandbox = create_session_sandbox(' in line and not line.rstrip().endswith(')'):
            ...
            line = line.replace('session_id, sandbox = create_session_sandbox(', 'sandbox = create_sandbox(')
            ...
            if var_match:
                ...
                while i < len(lines):
                    ...
                    if 'session_id,' in param_line or 'session_id' in param_line:
                        ...
                        param_line = re.sub(r'(\s+)session_id,', r'\1session_id=session_id,', param_line)
