...
class ExecutionPolicy(BaseModel):
    ...
    stdout_max_bytes: int = Field(
        default=2_000_000, gt=0, description="Maximum stdout capture size"
    )

    stderr_max_bytes: int = Field(
        default=1_000_000, gt=0, description="Maximum stderr capture size"
    )

    mount_host_dir: str = Field(default="workspace", description="Host directory for WASI preopen")

    guest_mount_path: str = Field(default="/app", description="Guest-visible mount point")

    mount_data_dir: str | None = Field(
        default=None, description="Optional secondary mount for read-only data"
    )

    guest_data_path: str | None = Field(
        default=None, description="Mount point for optional data directory"
    )
    ...
    timeout_seconds: float | None = Field(
        default=None, ge=0, description="Optional OS-level timeout (None = no timeout)"
    )
...
class SandboxResult(BaseModel):
    ...
    stdout: str = Field(default="", description="Captured stdout (may be truncated per policy)")

    stderr: str = Field(default="", description="Captured stderr (may be truncated per policy)")

    exit_code: int = Field(default=0, description="Guest process exit code (0 = success)")

    fuel_consumed: int | None = Field(
        default=None, description="WASM instructions executed (None if not tracked)"
    )

    memory_used_bytes: int = Field(default=0, description="Peak memory usage in bytes")

    duration_ms: float = Field(default=0.0, description="Wall-clock execution time in milliseconds")

    files_created: list[str] = Field(
        default_factory=list, description="New files in workspace (relative paths)"
    )

    files_modified: list[str] = Field(
        default_factory=list, description="Modified files in workspace (relative paths)"
    )
